...
class HMC():
    def __init__(self, models, S, B, T, K, z_where_dim, z_what_dim, hmc_num_steps, step_size_what, step_size_where, leapfrog_num_steps, device):
        self.models = models
        self.S = S
        self.B = B
        self.T = T
        ...
        self.uniformer = Uniform(torch.Tensor([0.0]).to(device), torch.Tensor([1.0]).to(device))
    ...
    def init_sample(self):
        ...
        """
        return self.gauss_dist.sample((self.S, self.B, self.T, self.K, self.z_where_dim,)).squeeze(-1), self.gauss_dist.sample((self.S, self.B, self.T, self.K, self.z_what_dim, )).squeeze(-1)
    ...
    def hmc_sampling(self, ob, z_where, z_what, trace):
        ...
        for m in range(self.hmc_num_steps):
            ...
            log_joint = self.models['dec'].log_joint_density(frames=ob, z_where=z_where, z_what=z_what)
    ...
    def metrioplis(self, ob, z_where, z_what):
        ...
        accept_index_expand2 = accept_index.unsqueeze(-1).unsqueeze(-1).repeat(1, 1, self.K, self.z_where_dim)
    ...
    def leapfrog(self, ob, z_where, z_what, r_where, r_what):
        for step in range(self.lf_num_steps):
            ...
            log_p = self.models['dec'].log_joint_density(frames=ob, z_where=z_where, z_what=z_what)
    ...
    def hamiltonian(self, ob, z_where, z_what, r_where, r_what):
        ...
        Uq = self.models['dec'].log_joint_density(frames=ob, z_where=z_where, z_what=z_what)
