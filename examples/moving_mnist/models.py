...
class Enc_digit(nn.Module):
    ...
    def bpg(self, dec, q, frames, extend_dir):
        z_where = []
        for t in range(frames.shape[2]):
            z_where.append(q['z_where_%d' % (t+1)].value.unsqueeze(2))
        z_where = torch.cat(z_where, 2)        
        ...
        cropped = self.AT.frame_to_digit(frames=frames, z_where=z_where)
        cropped = torch.flatten(cropped, -2, -1)
        q_mu = dec.prior_what_mu.repeat(S, B, K, 1)
        q_std = dec.prior_what_std.repeat(S, B, K, 1)
        q_new = copy_trace(q, 'z_what')
        if extend_dir == 'forward':
            if self.reparameterized:
                z_what = Normal(q_mu, q_std).rsample()
...
class Decoder(nn.Module):
    ...
    def __init__(self, num_pixels, num_hidden, z_where_dim, z_what_dim, AT, device):
        ...
        self.prior_where0_mu = torch.zeros(z_where_dim).to(device)
        self.prior_where0_Sigma = torch.ones(z_where_dim).to(device) * 1.0
        self.prior_wheret_Sigma = torch.ones(z_where_dim).to(device) * 0.2
        self.prior_what_mu = torch.zeros(z_what_dim).to(device)
        self.prior_what_std = torch.ones(z_what_dim).to(device)
    ...
    def forward(self, q, frames, recon_level, timestep=None):
        ...
        if recon_level == 'object': ## return the recnostruction of objects
            ...
            ll = self.MBern_log_prob(x_mean=recon_frames, x=frames[:,:,timestep,:,:])
            return p, ll, recon_frames.detach()
            ...
            ll = self.MBern_log_prob(x_mean=recon_frames, x=frames).sum(-1)
            return p, ll, recon_frames.detach()
        else:
            raise ValueError
    ...
    def log_prior(self, frames, z_where, z_what):
        ...
        log_p += self.MBern_log_prob(x_mean=recon_frames, x=frames)
        return log_p
    
    def MBern_log_prob(self, x_mean, x, EPS=1e-9):
        """
        the size is ... * H * W
