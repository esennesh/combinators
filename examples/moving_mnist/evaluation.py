...
def viz_samples(frames, metrics, num_sweeps, K, object_size, fs=1, title_fontsize=12, lw=2, colors=['#AA3377', '#EE7733', '#009988', '#0077BB', '#BBBBBB', '#EE3377', '#DDCC77'], save=False):
    B, T, FP, _ = frames.shape
    recons_first = metrics['E_recon'][0].cpu() # B * T * 96 *96
    z_wheres_first = metrics['E_where'][0].cpu().clone()
    ...
    recons_last = metrics['E_recon'][-1].cpu() # B * T * 96 *96
    z_wheres_last = metrics['E_where'][-1].cpu().clone()
    ...
    for b in range(B):
        ...
        for c in range(num_cols):
            ...
            ax_infer = fig.add_subplot(gs[2, c])
            ax_infer.imshow(frames[b, c].numpy(), cmap='gray', vmin=0.0, vmax=1.0)
            ax_infer.set_xticks([])
            ax_infer.set_yticks([])
            for k in range(K):
                ...
                ax_infer.add_patch(rect_k)
            ax_recon = fig.add_subplot(gs[3, c])
